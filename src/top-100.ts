...
const getTokenLogo = (address: string): string => {
  ...
  if (logoFiles.includes(`${address}.png`)) {
    return "https://nobleswap.io/logo.png";  // original form `https://tokens.pancakeswap.finance/images/${address}.png`
  }

  return "https://nobleswap.io/logo.png"; // original form `https://assets.trustwallet.com/blockchains/smartchain/assets/${address}/logo.png`
};
...
const main = async (): Promise<void> => {
  try {
    ...
    const tokenListPath = `${path.resolve()}/src/tokens/nobleswap-gil-top-100.json`;
